...
interface TestData {
    ...
    errorStack?: string;
    tags: string[];
    consoleLogs: string[];   // all stdout + stderr lines for the test
}
...
class CustomTTAReporter implements Reporter {
    ...
    onTestEnd(test: TestCase, result: TestResult): void {
        ...
        const testData: TestData = {
            ...
            tags: tagMatches,
            consoleLogs: this.collectConsoleLogs(result),
        };
    ...
    /** Collect all stdout + stderr lines from a test result into a flat string array. */
    private collectConsoleLogs(result: TestResult): string[] {
        const logs: string[] = [];
        ...
        for (const chunk of result.stderr || []) {
            toText(chunk).split('\n').filter((l: string) => l.trim()).forEach((l: string) => logs.push(`[stderr] ${l}`));
        }
...
<body>
    ...
    private generateTestDetailPanel(test: TestData): string {
        ...
        if (test.steps.length > 0) {
            ...
            <div class="detail-section steps-section">
                <div class="section-header" onclick="toggleSection(this)">
                    ...
                    <span class="section-badge">${test.steps.length} steps</span>
                    <span class="section-badge passed-badge">✅ ${passedSteps}</span>
                    ${failedSteps > 0 ? `<span class="section-badge failed-badge">❌ ${failedSteps}</span>` : ''}
                </div>
                <div class="section-content">
            ...
            for (let stepIndex = 0; stepIndex < test.steps.length; stepIndex++) {
                ...
                const hasDetails = (step.consoleLogs && step.consoleLogs.length > 0)
                    || step.screenshot || step.error;

                html += `
                    <div class="step-item-container">
                        <div class="step-item ${stepClass} ${hasDetails ? 'expandable' : ''}"
                             ${hasDetails ? `onclick="toggleStepDetails(this, '${stepId}-details')"` : ''}>
                            <span class="step-expand-icon">${hasDetails ? '▶' : ' '}</span>
                            ...
                            <span class="step-video-ts">🎬 ${this.formatVideoTime(step.videoStartTime || 0)}</span>
                        </div>
                        <div id="${stepId}-details" class="step-details" style="display: none;">
                            <div class="step-meta">
                                ...
                                <span class="step-meta-item">🎬 Video timestamp: ${this.formatVideoTime(step.videoStartTime || 0)} → ${this.formatVideoTime(step.videoEndTime || 0)}</span>
                            </div>`;
                ...
                if (step.consoleLogs && step.consoleLogs.length > 0) {
                    html += `
                            <div class="step-console">
                                <div class="step-console-header">📋 Console Output
                                    <span class="console-count-badge">${step.consoleLogs.length} lines</span>
                                </div>
                                <div class="step-console-content">`;
                    for (const log of step.consoleLogs) {
                        const logClass = log.startsWith('[stderr]') ? 'console-line stderr-line' : 'console-line';
                ...
                if (step.screenshot) {
                    html += `
                            <div class="step-screenshot">
                                ...
                                <a href="${step.screenshot}" target="_blank" class="screenshot-link">
                                    <img src="${step.screenshot}" alt="Step ${stepIndex + 1} Screenshot" class="step-screenshot-img"/>
                                </a>
                            </div>`;
                }
                ...
                if (step.error) {
                    html += `
                            <div class="step-error">
                                <div class="step-error-header">❌ Step Error</div>
        ...
        // ── CONSOLE LOGS (test-level) ──────────────────────────────────────────
        if (test.consoleLogs && test.consoleLogs.length > 0) {
            const stderrCount = test.consoleLogs.filter(l => l.startsWith('[stderr]')).length;
            const stdoutCount = test.consoleLogs.length - stderrCount;
            html += `
            <div class="detail-section console-section">
                <div class="section-header" onclick="toggleSection(this)">
                    <span class="section-arrow">▼</span>
                    <span>📋 Console Logs</span>
                    <span class="section-badge">${test.consoleLogs.length} lines</span>
                    ${stdoutCount > 0 ? `<span class="section-badge stdout-badge">stdout ${stdoutCount}</span>` : ''}
                    ${stderrCount > 0 ? `<span class="section-badge stderr-badge">stderr ${stderrCount}</span>` : ''}
                </div>
                <div class="section-content">
                    <div class="console-toolbar">
                        <input type="text"
                               ...
                               oninput="filterConsoleLogs(this, '${test.id}')" />
                        <label class="console-toggle-label">
                            <input type="checkbox" checked
                                   onchange="toggleStdout(this, '${test.id}')"> stdout
                        </label>
                        <label class="console-toggle-label">
                            <input type="checkbox" checked
                                   onchange="toggleStderr(this, '${test.id}')"> stderr
                        </label>
                    </div>
                    <div class="console-output" id="console-${test.id}">`;

            for (let i = 0; i < test.consoleLogs.length; i++) {
                const log = test.consoleLogs[i];
                const isStderr = log.startsWith('[stderr]');
                const lineClass = isStderr ? 'console-line stderr-line' : 'console-line stdout-line';
                html += `<div class="${lineClass}" data-log-type="${isStderr ? 'stderr' : 'stdout'}">` +
                    `<span class="console-line-num">${i + 1}</span>` +
                    `<span class="console-line-text">${this.escapeHtml(log)}</span>` +
                    `</div>`;
            }
        ...
        if (test.screenshots.length > 0) {
            ...
            <div class="detail-section screenshots-section">
                <div class="section-header" onclick="toggleSection(this)">
                    ...
                    <span class="section-badge">${test.screenshots.length} image${test.screenshots.length > 1 ? 's' : ''}</span>
                </div>
                <div class="section-content">
                ...
                html += `
                    <div class="screenshot-item">
                        <a href="${screenshot.path}" target="_blank" class="screenshot-link">
                            <img src="${screenshot.path}" alt="${this.escapeHtml(screenshot.name)}" class="screenshot-preview"/>
                            <div class="screenshot-overlay">🔍 View Full</div>
                        </a>
        ...
        if (test.video) {
            ...
            <div class="detail-section videos-section">
                <div class="section-header" onclick="toggleSection(this)">
                    ...
                    <span class="section-badge">webm</span>
                </div>
                <div class="section-content">
                    <div class="video-container">
                        <video id="video-${test.id}" controls class="test-video" preload="metadata">
                            <source src="${test.video}" type="video/webm">
                            Your browser does not support the video tag.
                        </video>
                        <div class="video-controls-extra">
                            <a href="${test.video}" download class="video-download-btn">⬇️ Download Video</a>
                            <a href="${test.video}" target="_blank" class="video-open-btn">↗️ Open in Tab</a>
                        </div>
                    </div>
                    ${test.steps.length > 0 ? `
                    <div class="video-step-markers">
                        <div class="video-markers-title">⏱️ Jump to step:</div>
                        <div class="video-markers-list">
                            ${test.steps.map((step, i) => `
                            <button class="video-marker-btn"
                                    onclick="seekVideo('video-${test.id}', ${(step.videoStartTime || 0) / 1000})"
                                    title="${this.escapeHtml(step.title)}">
                                ${i + 1}. ${this.escapeHtml(step.title.substring(0, 30))}${step.title.length > 30 ? '…' : ''}
                                <span class="marker-ts">${this.formatVideoTime(step.videoStartTime || 0)}</span>
                            </button>`).join('')}
                        </div>
                    </div>` : ''}
                </div>
        ...
        if (test.trace) {
            ...
            <div class="detail-section traces-section">
                <div class="section-header" onclick="toggleSection(this)">
                    <span class="section-arrow">▼</span>
                    <span>🔍 Trace</span>
                    <span class="section-badge">zip</span>
                </div>
                <div class="section-content">
                    <div class="trace-panel">
                        <div class="trace-actions">
                            <a href="${test.trace}" download class="trace-download">
                                ⬇️ Download Trace (.zip)
                            </a>
                            <a href="${test.trace}" target="_blank" class="trace-open-btn">
                                ↗️ Open File
                            </a>
                        </div>
                        <div class="trace-instructions">
                            <div class="trace-instructions-title">📖 How to view this trace:</div>
                            <ol class="trace-steps-list">
                                <li>Download the <code>.zip</code> file above</li>
                                <li>Run: <code class="trace-cmd">npx playwright show-trace &lt;path-to-trace.zip&gt;</code></li>
                                <li>Or drag &amp; drop the zip onto <a href="https://trace.playwright.dev" target="_blank" class="trace-link">trace.playwright.dev</a></li>
                            </ol>
                        </div>
                    </div>
                </div>
    ...
    private getStyles(): string {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .passed-badge { background: #dcfce7; color: #15803d; }
        .failed-badge { background: #fee2e2; color: #b91c1c; }
        .stdout-badge { background: #dbeafe; color: #1d4ed8; }
        .stderr-badge { background: #fef3c7; color: #b45309; }
        ...
        .stderr-line { color: #f59e0b !important; }
        ...
        .console-filter-input:focus { border-color: var(--primary); }
        ...
        .console-toggle-label input { accent-color: var(--primary); }
        .console-output {
            background: #0f172a;
            border-radius: var(--radius-sm);
            padding: 16px;
        ...
        .console-line:last-child { border-bottom: none; }
        .console-line.stdout-line { color: #a7f3d0; }
        .console-line.stderr-line { color: #fbbf24; }
        .console-line-num {
            color: #475569;
            min-width: 32px;
            text-align: right;
        ...
        .console-line-text { word-break: break-all; }
        .console-line.hidden { display: none; }

        /* ========== SCREENSHOT OVERLAY ========== */
        .screenshot-item { position: relative; }
        .screenshot-link { position: relative; display: block; }
        .screenshot-overlay {
            position: absolute;
            inset: 0;
        ...
        /* ========== VIDEO ========== */
        .video-container { display: flex; flex-direction: column; gap: 14px; }
        .video-controls-extra {
            display: flex;
            gap: 12px;
            flex-wrap: wrap;
        }
        .video-download-btn, .video-open-btn {
            display: inline-flex;
            align-items: center;
            gap: 6px;
            padding: 8px 16px;
        ...
        .video-download-btn:hover { background: #f59e0b; color: white; }
        .video-open-btn {
            background: var(--gray-100);
            color: var(--gray-600);
            border: 1px solid var(--gray-200);
        }
        .video-open-btn:hover { background: var(--gray-200); }
        .video-step-markers {
            border-top: 1px solid var(--gray-100);
            padding-top: 16px;
        }
        ...
        .video-marker-btn:hover { background: var(--primary); color: white; }
        .marker-ts {
            font-family: 'JetBrains Mono', monospace;
            font-size: 10px;
        ...
        /* ========== TRACE ========== */
        .trace-panel { display: flex; flex-direction: column; gap: 20px; }
        .trace-actions { display: flex; gap: 12px; flex-wrap: wrap; align-items: center; }
        .trace-open-btn {
            display: inline-flex;
            align-items: center;
            gap: 6px;
        ...
        .trace-open-btn:hover { background: var(--gray-200); }
        .trace-instructions {
            background: var(--gray-50);
            border: 1px solid var(--gray-200);
            border-radius: var(--radius-sm);
    ...
    private getScripts(): string {
        ...
        // ── Console log helpers ────────────────────────────────────────────────
        function filterConsoleLogs(input, testId) {
            const query = input.value.toLowerCase();
            const container = document.getElementById('console-' + testId);
            if (!container) return;
            container.querySelectorAll('.console-line').forEach(line => {
                const text = line.querySelector('.console-line-text')?.textContent?.toLowerCase() || '';
                line.classList.toggle('hidden', query.length > 0 && !text.includes(query));
            });
        }

        function toggleStdout(checkbox, testId) {
            const container = document.getElementById('console-' + testId);
            if (!container) return;
            container.querySelectorAll('.stdout-line').forEach(line => {
                line.classList.toggle('hidden', !checkbox.checked);
            });
        }

        function toggleStderr(checkbox, testId) {
            const container = document.getElementById('console-' + testId);
            if (!container) return;
            container.querySelectorAll('.stderr-line').forEach(line => {
                line.classList.toggle('hidden', !checkbox.checked);
            });
        }
